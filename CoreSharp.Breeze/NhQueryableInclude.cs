...
namespace CoreSharp.Breeze
{
    ...
    ///     Include supports the OData $expand implementation, which retrieves related entities by following navigation
    ///     properties.
    ...
    public class NhQueryableInclude<T> : NhQueryable<T>, IQueryableInclude
    {
        ...
        /// </summary>
        /// <remarks>
        ///     Note that some combinations of operations (.Select with .Where) result in errors
        ///     from the NHibernate LINQ provider when using cacheable queries.
        /// </remarks>
        ...
        public NhQueryableInclude(ISessionImplementor si, bool cacheable = false)
            : base(new DefaultQueryProvider(si),
                Expression.Constant(new NhQueryable<T>(si).WithOptions(x => { x.SetCacheable(cacheable); })))
        {
        }
        ...
        ///     Cache Region to use for caching the query.
        ///     <see cref="http://nhforge.org/doc/nh/en/#performance-querycache" /> and
        ///     <see cref="http://nhforge.org/doc/nh/en/#caches" />
        /// </param>
        public NhQueryableInclude(ISessionImplementor si, string cacheRegion)
            : base(new DefaultQueryProvider(si), Expression.Constant(new NhQueryable<T>(si).WithOptions(x =>
            {
                x.SetCacheable(true);
                x.SetCacheRegion(cacheRegion);
            })))
        {
        }
        ...
        /// </summary>
        /// <example>
        ///     var query = new NhQueryableInclude
        ///     <Customer>
