...
namespace CoreSharp.Breeze.Serialization
{
    public class SerializationModelRule<TModel> : SerializationModelRule, ISerializationModelRule<TModel>
    {
        ...
        public ISerializationModelRule<TModel> ForProperty<TProperty>(
            Expression<Func<TModel, TProperty>> propExpression,
            Action<ISerializationMemberRule<TModel, TProperty>> action)
        {
            ...
            if (propInfo == null)
            {
                throw new NullReferenceException(
                    string.Format("Type '{0}' does not contain a property with name '{1}'.", ModelType, propName));
            }
        ...
        public ISerializationModelRule<TModel> CreateProperty<TProperty>(string propName,
            Func<TModel, TProperty> propValFunc)
        {
            if (propValFunc != null)
            {
