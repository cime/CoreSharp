...
namespace CoreSharp.Breeze
{
    ...
    public class DynamicTypeInfo
    {
        ...
        /// For internal use only.
        /// </summary>
        public ReadOnlyCollection<Type> PropertyTypes => _propertyTypes.AsReadOnly();
        ...
        /// </summary>
        public bool DynamicTypeShouldSave { get; set; }

        /// <summary>
        /// File directory to write out the dynamic types assembly.
        /// </summary>
        public string DynamicTypeFileDirectory { get; set; }
        ...
        {
            var dynamicTypeName = ConvertDynamicAssemblyNameToDynamicTypeName(pAssemblyName);
        ...
        {
            var ix = pAssemblyName.IndexOf(DynamicAssemblyNameSuffix);
            if (ix == -1) return string.Empty;
            var typeName = pAssemblyName.Substring(0, ix);
            return typeName;
        }
        ...
        private string GetUniqueToken()
        {
            ...
            var input = (TypeName ?? "") + "|" + PropertyNames.ToAggregateString(",") + "|" +
                        PropertyTypes.Select(t => t.FullName).ToAggregateString(",");
        ...
        private static string CalcStringHash(string stringToHash, int maxChars)
        {
            ...
            var byteContents = Encoding.Unicode.GetBytes(stringToHash);
            var provider = new System.Security.Cryptography.SHA1Managed();
            var hash = provider.ComputeHash(byteContents);
        ...
        {
            var info = FindByAssemblyName(pArgs.Name);
            if (info == null)
            {
